...
export function ProposerDatasetsPage() {
  ...
  const allTags = [...new Set(published.flatMap((d) => d.tags))].sort();
  const allDepts = [...new Set(published.map((d) => {
  ...
  const recentCount = published.filter(d => new Date(d.created_at) > oneMonthAgo).length;
  const totalTables = published.reduce((s, d) => s + getDatasetSampleTables(d.dataset_id).length, 0);
  const totalUseCases = published.reduce((s, d) => s + getDatasetUseCases(d.dataset_id).length, 0);
  ...
  const filtered = published.filter((d) => {
    ...
    const matchDept = selectedDepts.length === 0 || selectedDepts.includes(ownerDept);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-6 bg-white rounded-lg shadow-sm px-5 py-3 mb-6 text-sm">
        <div><span className="text-gray-500">公開データセット:</span> <span className="font-bold text-blue-600">{published.length}件</span></div>
      ...
      <div className="flex gap-6">
        ...
        <aside className="w-60 shrink-0">
          <div className="bg-white rounded-lg shadow-sm p-4 sticky top-4 space-y-5">
            ...
            {(selectedTags.length > 0 || selectedDepts.length > 0) && (
              <button
                onClick={() => { setSelectedTags([]); setSelectedDepts([]); }}
                className="text-xs text-blue-500 hover:text-blue-700"
              >
                フィルタをクリア ({selectedTags.length + selectedDepts.length}件選択中)
              </button>
            )}
          </div>
        </aside>
            ...
            {filtered.map((d) => {
              ...
              return (
                <Link
                  ...
                  className="bg-white rounded-lg shadow p-5 border-t-[3px] border-blue-500 hover:-translate-y-1 hover:shadow-lg transition-all duration-300 block"
                >
